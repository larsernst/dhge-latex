...
\if\CKUSCHE 1
    ...
    % Anhänge werden mit Großbuchstaben A, B, C, ... nummeriert
    % und wie normale Hauptkapitel im Inhaltsverzeichnis gelistet.
    % Abbildungen/Tabellen in den Anhängen heißen dann A.1, A.2, B.1 usw.
    % Verwendung in anlagen.tex:
    ...
    \newcommand{\anhang}[1]{%
        ...
        \setcounter{figure}{0}%
        \setcounter{table}{0}%
        \renewcommand{\thefigure}{\Alph{anhang}.\arabic{figure}}%
        \renewcommand{\thetable}{\Alph{anhang}.\arabic{table}}%
        \phantomsection
        \section*{Anhang \Alph{anhang}: #1}%
        \addcontentsline{toc}{section}{\protect\numberline{\Alph{anhang}}#1}%
    }
\fi
...
% Literaturverzeichnis: Hanging Indent (Standard von biblatex), Flattersatz
% (verhindert unschöne Wort-Zwischenräume und Umbruch mitten in URLs)
...
% Fußnoten: Schrift 1 pt kleiner als Fließtext (12 pt -> 11 pt), einfacher Zeilenabstand
...
% GLOSSAR / ABKÜRZUNGSVERZEICHNIS SETUP
% CGLOSSAR=1: Glossar (Fachbegriffe + Abkürzungen, alphabetisch sortiert,
% Fachbegriff fett auf eigener Zeile, Erläuterung eingerückt darunter)
% anstelle des Abkürzungsverzeichnisses.
% Fachbegriffe werden mit \newglossaryentry{...}{...} und Abkürzungen mit
% \newacronym{...}{...}{...} in glossar.tex definiert.
% Im Text werden sie mit \gls{...} aufgerufen (bzw. \ac{...}).
% CGLOSSAR=0: klassisches Abkürzungsverzeichnis via acro-Paket (abk.tex).
\if\CGLOSSAR 1
    % Formatierung: Fachbegriff fett auf eigener Zeile, Erläuterung als
    % eingerückter Block darunter. Flattersatz (\RaggedRight) verhindert
    % unschöne Wort-Zwischenräume und komische Zeilenumbrüche.
    \renewcommand*{\glsnamefont}[1]{\textbf{#1}}
    \newglossarystyle{dhge}{%
        ...
        \renewcommand{\glossentry}[2]{%
            ...
            \RaggedRight\glossentrydesc{##1}\glspostdescription}%
        \renewcommand{\subglossentry}[3]{\glossentry{##1}{##3}}%
    }
    \setglossarystyle{dhge}

    % Kompatibilität zum bisherigen \ac-Befehl des acro-Pakets:
    % \ac{id} entspricht \gls{id} (erste Nutzung: lang (KURZ), danach KURZ)
    \let\ac\gls
    \let\acp\glspl
